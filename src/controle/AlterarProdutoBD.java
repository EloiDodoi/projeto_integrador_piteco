...
public class AlterarProdutoBD {
	ConexaoBD cbd = new ConexaoBD();
	 public void alterarProduto(Produto p) {
		 try {
			PreparedStatement ps = cbd.getConexao().prepareStatement("UPDATE  produto_nomeveg = ?, "
					+ "produto_especieveg = ?,produto_preco = ?, produto_quantidade=?"
					+ ", Unidade_quantidade_idUnidade_quantidade = ?   where set idproduto = ?");
			...
			ps.setInt(3, p.getUnidade_quantidade_idUnidade_quantidade());
